...
def read_data() -> pd.DataFrame:
    ...
    for i in range(len(values["ReadGray(s)"])):
        ...
        df = pd.DataFrame(columns=["Node", "Processes", "Num Threads", "ReadGray(s)", "ReadColor(s)", "Gray(s)", "Hsl(s)", "Yuv(s)", "WriteGray(s)", "WriteHsl(s)", "WriteYuv(s)", "Total(s)"])
