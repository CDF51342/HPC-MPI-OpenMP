...
def read_node_data(number: int) -> pd.DataFrame:
    ...
    data = data.groupby(['Nodes', 'Processes', 'Num Threads'], as_index=False).agg(confidence_interval).reset_index() 
...
def add_label_column(data: pd.DataFrame) -> pd.DataFrame:
    del data['index']
    data['Label'] = "MPI+OpenMP: N " + data['Nodes'].astype(str) + ", Prc " + data['Processes'].astype(str) + ", Threads " + data['Num Threads'].astype(str)
...
if __name__ == "__main__":
    try:
        ...
        print("\033[92m" + "[OK]" + "\033[0m" + " MPI + OpenMP data joined successfully")
    except Exception as e:
        print("\033[91m" + "[ERROR]" + "\033[0m" + " Error joining MPI + OpenMP data")
        print(e)
        raise
